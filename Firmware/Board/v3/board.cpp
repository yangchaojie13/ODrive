...
std::array<Axis, AXIS_COUNT> axes{{
    ...
    /* {
        ...
        trap[1],                   // trap
        endstops[2],
        endstops[3],           // min_endstop, max_endstop
        mechanical_brakes[1],  // mechanical brake
    }, */
}};
...
Stm32Gpio gpios[GPIO_COUNT] = {
    ...
    {nullptr, 0},  // 屏蔽 GPIO1 (PA0)
    {nullptr, 0},  // 屏蔽 GPIO2 (PA1)
    {nullptr, 0},  // 屏蔽 GPIO3 (PA2)
    {GPIOA, GPIO_PIN_3},   // GPIO4
...
#else
PwmInput pwm0_input{&htim5, {0, 0, 0, 0}};
#endif
...
bool board_init() {
    // // ====================================================
    // // 🛑 终极硬件隔离测试（软件点穴法）- 变量名已修复版
    // // ====================================================
    // // 1. 唤醒 DRV8323 (拉高 PB0)
    // __HAL_RCC_GPIOB_CLK_ENABLE();
    // GPIO_InitTypeDef GPIO_InitStruct_EN_Test = {0}; // <--- 改名了
    // GPIO_InitStruct_EN_Test.Pin = GPIO_PIN_0;
    // GPIO_InitStruct_EN_Test.Mode = GPIO_MODE_OUTPUT_PP;
    // GPIO_InitStruct_EN_Test.Speed = GPIO_SPEED_FREQ_LOW;
    // HAL_GPIO_Init(GPIOB, &GPIO_InitStruct_EN_Test);
    // HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0, GPIO_PIN_SET);

    // // 2. 强行把 PA0, PA1, PA2 设置为纯模拟输入（高阻态，彻底断开单片机的驱动）
    // __HAL_RCC_GPIOA_CLK_ENABLE();
    // GPIO_InitTypeDef GPIO_InitStruct_ADC_Test = {0}; // <--- 改名了
    // GPIO_InitStruct_ADC_Test.Pin = GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_2;
    // GPIO_InitStruct_ADC_Test.Mode = GPIO_MODE_ANALOG;
    // GPIO_InitStruct_ADC_Test.Pull = GPIO_NOPULL;
    // HAL_GPIO_Init(GPIOA, &GPIO_InitStruct_ADC_Test);

    // // 3. 脑死循环！系统直接卡死在这里，后面的代码全都不执行！
    // while(1) {
    //     // 就在这待着，哪也别去
    // }
    // // ====================================================
    ...
    // ====================================================
    // 🚀 拯救 DRV8323 专属苏醒与配置仪式
    // ====================================================
    
    // 1. 唤醒 DRV8323 (你的原理图上 DRV_EN 是 PB0)
    __HAL_RCC_GPIOB_CLK_ENABLE();
    GPIO_InitTypeDef GPIO_InitStruct_EN = {0};
    GPIO_InitStruct_EN.Pin = GPIO_PIN_0;
    GPIO_InitStruct_EN.Mode = GPIO_MODE_OUTPUT_PP;
    GPIO_InitStruct_EN.Speed = GPIO_SPEED_FREQ_LOW;
    HAL_GPIO_Init(GPIOB, &GPIO_InitStruct_EN);
    
    // 拉高 PB0，把芯片扇醒！
    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_0, GPIO_PIN_SET);
    delay_us(10000); // 必须等它醒来 (至少 1ms 以上)

    // 2. 软件 Bit-Bang 暴力注入 SPI1 (PA4=CS, PA5=SCK, PA6=MOSI)
    // 既然 ODrive 没帮我们初始化 SPI1，我们就自己手动发时钟！
    __HAL_RCC_GPIOA_CLK_ENABLE();
    GPIO_InitTypeDef GPIO_InitStruct_SPI = {0};
    GPIO_InitStruct_SPI.Pin = GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_7; // <--- 已经修正为 PA7
    ...
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET);   // CS 默认高
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_RESET); // SCK 默认低
    delay_us(10);

    // 准备发送的魔法指令：0x06寄存器，40V/V增益
    uint16_t spi_tx_buf = 0x32C3; 
    
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_RESET); // CS 拉低，开始通讯！
    delay_us(2);

    // 循环 16 次，把每一位数据塞进芯片
    for (int i = 15; i >= 0; i--) {
        if (spi_tx_buf & (1 << i)) {
            HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, GPIO_PIN_SET); // <--- 已经修正为 PA7
        } else {
            HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, GPIO_PIN_RESET); // <--- 已经修正为 PA7
        }
        delay_us(1);
        HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_SET);   // SCK 变高，芯片读入
        delay_us(1);
        HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_RESET); // SCK 变低，准备下一位
        delay_us(1);
    }

    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET); // CS 拉高，结束通讯！
    delay_us(10);
    // ====================================================
    

    // ====================================================
    // 🚀 终极引脚保卫战：等所有“土匪”都初始化完后，强行把 PA0, PA1, PA2 抢回给 ADC！
    // 这样既不会让 RTOS 崩溃，又能保证我们读到真实的 1.65V 基准电流！
    // ====================================================
    __HAL_RCC_GPIOA_CLK_ENABLE();
    GPIO_InitTypeDef GPIO_InitStruct_ADC = {0};
    GPIO_InitStruct_ADC.Pin = GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_2;
    GPIO_InitStruct_ADC.Mode = GPIO_MODE_ANALOG; // 强制设为模拟输入
...
void start_timers() {
    CRITICAL_SECTION() {
        ...
        // ====================================================
        // 🚀 终极核弹指令：正确修改 JSQR 寄存器
        // 只改 JSQ4 位，不破坏其他配置！
        // ====================================================
        // 清除 JSQ4 位（位 15-19），然后设置新通道号
        // ADC2->JSQR = (ADC2->JSQR & ~(0x1F << 15)) | (1 << 15);  // 通道 1 (PA1)
        // ADC3->JSQR = (ADC3->JSQR & ~(0x1F << 15)) | (2 << 15);  // 通道 2 (PA2)
        // ====================================================
